...
test.describe('Bottom Navigation', () => {
  test('should navigate between pages using bottom nav tabs', async ({ page }) => {
    ...
    // Click Trends tab in bottom navigation
    await page.getByRole('navigation').getByRole('link', { name: 'Trends', exact: true }).click();
    await expect(page).toHaveURL('/trends');
    
    // Click Home tab
    await page.getByRole('navigation').getByRole('link', { name: 'Home', exact: true }).click();
    await expect(page).toHaveURL('/demo'); // Unauthenticated users redirect to demo
    
    // Click Device tab
    await page.getByRole('navigation').getByRole('link', { name: 'Device', exact: true }).click();
    ...
    // Click Meds tab
    await page.getByRole('navigation').getByRole('link', { name: 'Meds', exact: true }).click();
